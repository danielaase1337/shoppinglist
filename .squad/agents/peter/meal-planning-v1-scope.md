# Meal Planning ÔÇö Full Implementation Plan

**Author:** Peter (Lead/Architect)  
**Date:** 2026-04-03 (Revised ÔÇö Full Scope + Refinements)  
**Supersedes:** Meal Planning v1 slim scope (2026-03-29)  
**Status:** Ô£à FINALIZED (2026-04-03) ÔÇö All phases locked, ready for Phase 1 implementation tickets  
**Issue:** #29 (Scoping: Meal Planning ÔÇö Full Implementation)  
**Blocks:** All meal planning implementation tickets

---

## Context ÔÇö Why This Document Exists
...
4. **Track pantry inventory** ÔÇö what is in stock, updated automatically when shopping lists complete
...
| Rule | Detail |
|------|--------|
| **Pizza Fridays** | Friday = **suggested** Pizza (auto-populated with top-scoring Pizza recipe, `IsSuggested = true`). User CAN override. `IsLocked` removed ÔÇö see D36. |
...
## Unit System (Cross-Cutting Concern ÔÇö Design First)
...
|------|-----------|-------|
| `gram` | gram | Weight ÔÇö spices, butter, cheese |
| `kilogram` | kilogram | Weight ÔÇö meat, vegetables |
| `liter` | liter | Volume ÔÇö milk, cream, broth |
| `deciliter` | desiliter | Volume (common in Norwegian recipes) |
| `tablespoon` | spiseskje (ss) | Volume ÔÇö oils, sauces |
| `tablespoon_half` | 1/2 ss | |
| `tablespoon_quarter` | 1/4 ss | |
| `teaspoon` | teskje (ts) | Volume ÔÇö spices, baking |
| `teaspoon_half` | 1/2 ts | |
| `teaspoon_quarter` | 1/4 ts | |
| `piece` | stykk / stk | Countable ÔÇö eggs, onions, cans |
| `piece_half` | 1/2 stk | e.g. half a broccoli |
| `piece_quarter` | 1/4 stk | |
| `pinch` | klype | Spices ÔÇö "a pinch of salt" |
...
- `MealUnit` lives in `Shared` ÔÇö used by both `MealIngredient` and `InventoryItem`
- Display names (Norwegian) resolved via a `MealUnitExtensions.ToNorwegian()` helper
- Fractional quantities encoded as separate enum values (not decimal fields) ÔÇö avoids floating-point UI issues
...
{
    Quick,    // Ôëñ20 min ÔÇö frozen, pre-prepped, takeout
    Normal,   // 20ÔÇô45 min ÔÇö typical weeknight
    Weekend   // 45+ min ÔÇö lasagne, slow-cook, special meals
}
```

#### `MealIngredient` (embedded in MealRecipe ÔÇö per D3)
...
public class MealIngredient
{
    ...
    [FirestoreProperty] public bool IsBasic { get; set; }        // Base ingredient flag (D42) ÔÇö e.g. oil, salt, butter assumed in stock
}
```
...
public class DailyMeal
{
    ...
    [FirestoreProperty] public string MealRecipeId { get; set; }       // ref ÔÇö per D3
    [FirestoreProperty] public bool IsSuggested { get; set; }          // true = auto-filled (e.g. Pizza Friday), overridable ÔÇö replaces IsLocked (D36)
...
#### `InventoryItem` (Firestore + DTO) ÔÇö New collection
...
`MealIngredient`, `DailyMeal`, and `FamilyMember` are **embedded** ÔÇö no separate collections.

### Existing Entities Extended

- `ShoppingList`: No changes needed ÔÇö generated lists use existing structure
- `ShopItem`: No changes ÔÇö referenced by `ShopItemId` in ingredients

### New Entities (Phase 5 ÔÇö Family Profile)

```csharp
// FamilyProfile (D39) ÔÇö Firestore: familyprofiles
[FirestoreData]
...
// FamilyMember ÔÇö embedded in FamilyProfile (D3 convention)
[FirestoreData]
...
// PortionRule (D39) ÔÇö Firestore: portionrules
[FirestoreData]
...
### Phase 1 ÔÇö Meals + Ingredients CRUD

**Goal:** Database of meals with structured ingredients. Fast to add, easy to maintain.

**Scope:**
- `MealRecipe` CRUD ÔÇö API endpoints + admin UI page
...
- **`MealType` tagging** ÔÇö FreshCook / Frozen / PrePrepped / Takeout (D38)
- **`PrepTimeMinutes` + `MealEffort`** ÔÇö raw prep time + Quick/Normal/Weekend bucket (D37)
...
**Pages:**
- `Client/Pages/Meals/MealManagementPage.razor` ÔÇö List with search/filter by category
- `Client/Pages/Meals/OneMealRecipePage.razor` ÔÇö Recipe editor with ingredient list

**Dependencies:** Unit system enum must be in `Shared` before coding starts.

---

### Phase 2 ÔÇö Meal Planning + Week View
...
- **Friday slot: auto-populated with top-scoring Pizza recipe, `IsSuggested = true`, fully overridable** (D36 ÔÇö `IsLocked` removed)
- Recipe picker per day ÔÇö sorted by `PopularityScore`, filtered by category and `MealEffort`
...
- Navigation: "Ukemeny" added to main nav (not admin dropdown ÔÇö this is a primary feature)
...
**Shopping List Generation Logic (D42 ÔÇö Basevarer Feature):**
...
3. Check `InventoryItem` stock ÔÇö deduct available quantities from required amounts (primary only; basevarer pre-checked but user-selects)
...
5. Output as `ShoppingListModel` with:
   ...
   - Basevarer pre-checked in UI but NOT auto-added ÔÇö user reviews and selects what to include before finalizing list

**Basevarer Workflow (User Experience):**
- User clicks "Generate shopping list"
- System shows two sections: "Ingredienser denne uken" (Primary) and "Basevarer ÔÇö huk hva du trenger" (Suggested Base Items)
- Basevarer items are pre-checked with checkboxes ÔÇö user can uncheck to exclude or add manually to checked items
...
**Dependencies:** Phase 1 complete. Phase 4 inventory must exist for stock deduction ÔÇö if Phase 4 not done yet, generate full list without deduction.

---

### Phase 3 ÔÇö Ingredient Matching + "Use Up" Logic
...
- Suggestion is a nudge only ÔÇö user dismisses or accepts
...
### Phase 4 ÔÇö Inventory / Stock System

**Goal:** Track what is in the pantry. Auto-deduct when shopping lists complete.

**Scope:**
- `InventoryItem` CRUD ÔÇö admin UI page (grouped by item category)
...
- **"Cook double ÔåÆ freeze one" workflow (D38):**
  ...
  - No new Firestore collection ÔÇö frozen meals are inventory entries
...
```csharp
// In ShoppingListController.PUT ÔÇö when IsDone transitions false -> true
...
### Phase 5 ÔÇö Family Profile + Portion Scaling
...
**New entities:**
```csharp
// FamilyProfile ÔÇö one per household (D39)
[FirestoreData]
public class FamilyProfile : EntityBase
{
    [FirestoreProperty] public ICollection<FamilyMember> Members { get; set; }
...
// PortionRule ÔÇö separate Firestore collection: portionrules
...
1. Load `FamilyProfile` ÔåÆ count members by `AgeGroup` (e.g. 2 adults, 1 child, 1 toddler)
2. For each ingredient in aggregated list, check for a `PortionRule` matching `ShopItemId` + `AgeGroup`
3. If rule found: `totalQuantity = ╬ú (rule.QuantityPerPerson ├ù memberCount[ageGroup])`
4. If no rule: use raw recipe quantity as-is

**Scaling mode (D41 ÔÇö FINAL):** Semi-automatic. The system calculates `scalingFactor = FamilyMemberCount / MealRecipe.BasePortions` and pre-fills suggested scaled quantities. User can adjust any quantity before saving the list. If no `FamilyProfile` exists, recipe quantities are used as-is.
...
4. Review step shows suggested quantities ÔÇö user can override any line item
5. User confirms ÔåÆ `ShoppingListModel` saved with finalised quantities
...
**Pages:**
- `Client/Pages/Family/FamilyProfilePage.razor` ÔÇö family members + portion rules editor
...
2. Team member (Josh recommended ÔÇö clean structured data task) parses the text into the bulk import JSON format
3. JSON is POSTed to `POST /api/mealrecipes/import` (Phase 1 endpoint)
4. Initial import creates `MealRecipe` entries with `Name` only ÔÇö `Ingredients` empty (`IsActive=true`)
...
| Feature | Reason |
|---------|--------|
| AI/ML-powered suggestions | Overkill ÔÇö frequency + ingredient matching is sufficient |
| Multi-user / family isolation | Deferred to auth Phase 2 (D2 ÔÇö OwnerId scope) |
...
- **Phase 3** is the last core feature ÔÇö requires accurate ingredient data from real usage
...
- [ ] "Cook double ÔåÆ freeze one" creates frozen inventory entry
...
**Refined by:** Peter (Lead/Architect), 2026-04-03 (refinements: D36ÔÇôD39 applied ÔÇö Pizza Friday rule reversal, MealEffort/PrepTime, MealType/frozen workflow, Phase 5 FamilyProfile)  
**Finalized by:** Peter (Lead/Architect), 2026-04-03 (D40ÔÇôD42: BasePortions, semi-automatic scaling, full scope locked)  
**Reason for revision:** Daniel directive (issue #29, 2026-04-03) ÔÇö skip slim v1, go full implementation  
**Reason for refinement:** Daniel comment (issue #29, comment 4184857751) ÔÇö 4 scope refinements  
**Reason for finalization:** Daniel comment (issue #29, comment 4184947107) ÔÇö portion scaling confirmed semi-automatic  
**Reviewed by:** Daniel Aase (via GitHub comments)  
**Approved by:** Daniel Aase Ô£à
