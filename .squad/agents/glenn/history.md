...
- **Persistence bug is entirely client-side (cache), backend is clean**: For the shopping list item disappearance bug, the API PUT endpoint (`ShoppingListController`, lines 82–137) correctly does a full-document `SetAsync` replace via `GoogleFireBaseGenericRepository.Update()`. AutoMapper `ShoppingList ↔ ShoppingListModel` with `.ReverseMap()` correctly maps the full `ShoppingItems` collection. The bug lives in `OneShoppingListPage.razor:UpdateShoppinglist()` which never calls `DataCache.InvalidateShoppingListCache()` after PUT, so the 10-minute TTL cache returns stale data on navigation back.
- **`ShoppingListItem` has no `Id` — by design but has side effects**: `ShoppingListItem` (Firestore model) has NO `Id` property; it's embedded inside the `ShoppingList` document. `ShoppingListItemModel` inherits `Id` from `EntityBase`, but it's always null after mapping from Firestore. This means `DeleteAllClicked()` calling `Settings.GetApiUrlId(ShoppingListKeysEnum.ShopItem, item.Id)` always passes null and hits the wrong catalog endpoint — a separate, latent client-side bug.
- **`MemoryGenericRepository.Update()` is a full replacement**: `_data[entityToUpdate.Id] = entityToUpdate` replaces the entire entity. This mirrors `GoogleFireBaseGenericRepository.Update()` which uses `SetAsync` (no merge). Both repos behave identically — the entire `ShoppingItems` collection is always replaced on PUT, which is correct for this model.
